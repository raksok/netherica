...
Netherica is a Rust desktop ingestion/reporting application (egui/eframe) for Excel-based inventory flow. In its basic form a tools like Grist could probably handle this just fine. This is exist purely as me dipping my toe into the 'vibe coding' everybody talk about. 
...
- That hatred fuels paragraph is then pass to LLMs to draft a software requirements. The requirement was drafted with LLMs chat interface (not sophisticte agent conference) each was instruct to act as a group of senior engineers critic each other works, leveraging free tier as much as possible with human(me) as orchestrator. Main contributor obviously Sonet 4.6, Gemini 3.1 Pro and Gemini Thinking mode, GPT-chat(not sure about model), and Deepseek V3.2 (which surprisingly very logical). Then the LLMs then sliced it into Phase and Task, Thanks to plan mode.

- Coding loop consist of 3 agents, Main orchestrator agent, Engineer agent, and QA agent.
	- Orchestrator read the `TASKS.md` then read `REQUIREMENTS.md` picked the appropriate context then pass it to Engineer subagent to code.
	- Engineer code the specific section, run some basic `cargo check` then fixed the error until it's passed, then pass it to QA
	- QA run `cargo check`, `cargo clippy -- -D warnings`, `cargo fmt --check`, and `cargo test`. if fix needed it's pass back to Engineer, all test that is in requirement also apply at this step if all condition is to agents satisfactoy it return [APPROvE] message back to the orchestrator which then will move to next task.
...
The rest is LLMs generated. Basically you will be able to compile this just fine with with rust MSVC on Windows (YOYO if you're using MYSYS or MINGW) Linux version compiled and test on Fedora 43 KDE with stable rustup and development-tools group installed.
